# Modified source code for heatmap.2 function in R
...
326
 ...
 {
 flag <- try(
 plot.dendrogram(ddr, horiz = TRUE, axes = FALSE, yaxs = "i", 
leaflab = "none")
 )
 if("try-error" %in% class(flag))
 {
 cond <- attr(flag, "condition")
 if(!is.null(cond) && conditionMessage(cond)=="evaluation 
nested too deeply: infinite recursion / options(expressions=)?")
 stop('Row dendrogram too deeply nested, recursion limit 
exceeded. Try increasing option("expressions"=...).')
 }
 }
 else
 plot.new()
 ...
 if(!is.null(cond) && conditionMessage(cond)=="evaluation 
nested too deeply: infinite recursion / options(expressions=)?")
 stop('Column dendrogram too deeply nested, recursion 
limit exceeded. Try increasing option("expressions"=...).')
 }
 }
 else
 plot.new()
